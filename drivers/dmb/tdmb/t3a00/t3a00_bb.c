...
static boolean t3a00_function_register(tdmb_bb_function_table_type *);
...
SIDE EFFECTS
======================================================================*/
boolean tdmb_bb_t3a00_init(tdmb_bb_function_table_type *function_table_ptr)
{
  boolean bb_initialized;

  bb_initialized = t3a00_function_register(function_table_ptr);

  return bb_initialized;
}


static boolean t3a00_function_register(tdmb_bb_function_table_type *ftable_ptr)
{
...
void t3700_ch_scan_start(int freq, int band, unsigned char for_air)
{
  ...
  DMB_MSG_LOG("[%s] Channel Frequency[%d] Band[%d] Mode[%d]\n", __func__,freq, band, for_air);
  ...
  if(INC_SYNCDETECTOR(TMDB_I2C_CTRL_ID, freq, 0) == INC_SUCCESS)
  {
    ...
    DMB_MSG_LOG("[%s] SYNCDETECT SUCCESS\n",__func__);
  ...
  else
  {
    ...
    TDMB_MSG_INC_BB("[%s] SYNCDETECT ERROR  %x \n", __func__, ErrInfo);
...
{
  g_InCIntCtrl = INC_INTERRUPT_NON;
  TDMB_MSG_INC_BB("[%s] INC_INIT FUNCITON CALL [SDK Ver : 131011] \n", __func__);
  DMB_MSG_LOG("[%s] INC_INIT FUNCITON CALL [SDK Ver : 131011] \n", __func__);
...
//Need for SPI  
    ...
    else
    {
      ...
      DMB_MSG_LOG("[%s] OK  freq[%d] SubChID[%x] EID[%x]\n", __func__,pstChInfo->ulRFFreq,pstChInfo->ucSubChID,pstChInfo->uiEnsembleID);
      result = INC_SUCCESS;
    }
...
// 110127 (EBI2 : manual Resync, TSIF : HW AutoResync)
  ...
  DMB_MSG_LOG("snr(%d)pc ber(%d) rs ber(%d) ant(%d) RxPower(%d)\n", (int)psigs->SNR, (int)psigs->PCBER, (int)psigs->RSBER, (int)psigs->RSSI, psigs->RxPowerLevel);

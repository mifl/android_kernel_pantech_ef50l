...
#if defined(__KERNEL__) && defined(RTV_CIFDEC_DIRECT_COPY_USER_SPACE)\
/* Linux kernel */
...
enum {
	CIF_CONT_MED_MASK = (1 << CIF_CONT_MED),
	CIF_CONT_LAST_MASK = (1 << CIF_CONT_LAST),
	CIF_CONT_FIRST_MASK = (1 << CIF_CONT_FIRST),
	CIF_CONT_ALONE_MASK = (1 << CIF_CONT_ALONE)
} CIF_CONT_FLAG_BITMASK_TYPE;

/* ID flag */
enum CIF_ID_FLAG_TYPE {
	CIF_ID_DAB = 0,
	CIF_ID_DMB = 1,
	CIF_ID_FIDC = 2,
	CIF_ID_FIC = 3,
...
static enum CIF_ID_FLAG_TYPE g_eSingleSvcFlagID;
#endif

#define CIF_DISCARDED_TSP_MARK_0	0x48
#define CIF_DISCARDED_TSP_MARK_1	0xFF

#define MAX_CIF_PAYLOAD_SIZE (RTV_TSP_XFER_SIZE - CIF_HEADER_SIZE)

#define CIF_HEADER_SIZE		4 /* bytes */
	...
	*status = CIFDEC_S__OK;
	...
	while ((cifb = cifb_peek_frist(tspq)) != NULL) {
		...
		if (cifb->header_on == 0) {
			...
			if (cifb->cont == CIF_CONT_FIRST) {
				if (cifb->ts_ptr[0] == 0x47) {
					if (tspq->cnt > 1) {
						if (cifb->ptNext->cont == CIF_CONT_LAST) {
				...
				struct CIFB_INFO *cifb, enum CIFDEC_STATUS_TYPE *status)
{
	...
	if (tspq->cnt >= 3) {
		if ((cifb->ptNext->cont == CIF_CONT_MED)
		&& (cifb->ptNext->ptNext->cont == CIF_CONT_LAST)) {
			dec->fic_size = cifb->data_len; /* FINISHED */
...
static void proc_fic(struct RTV_CIF_DEC_INFO *dec, struct CIFB_HEAD_INFO *tspq)
{
	...
	while ((cifb = cifb_peek_frist(tspq)) != NULL) {
		...
		if (!dec->fic_size) { /* Decoding was not finished ? */
			...
			else if (cifb->cont == CIF_CONT_ALONE) {
				dec->fic_size = cifb->data_len; /* FINISHED */
...
#endif /* #ifdef RTV_FIC_CIFMODE_ENABLED */
	...
	decode_header(struct CIFB_INFO *cifb, const U8 *tsp)
{
	...
	if (tsp[0] == RTV_CIFDEC_HEADER_SYNC_BYTE) { /* sync byte ok? */
		...
		last_size = cifb->data_len % MAX_CIF_PAYLOAD_SIZE;
		if (!last_size)
			last_size = MAX_CIF_PAYLOAD_SIZE;

		switch (cifb->cont) {
		case CIF_CONT_FIRST:
		case CIF_CONT_MED:
#ifdef _DEBUG_HEADER_DECODING
			if (cifb->data_len < MAX_CIF_PAYLOAD_SIZE)
				return CIFDEC_S__SHORT_PAYLOAD_SIZE;
#endif
			cifb->ts_size = MAX_CIF_PAYLOAD_SIZE;
			break;
		case CIF_CONT_LAST:
		...
		case CIF_ID_FIC:
			...
			break;
#else
			return CIFDEC_S__NOT_EN_FIC_DECODER;
#endif
...
#if (RTV_MAX_NUM_USE_SUBCHANNEL == 1)
		if (g_eSingleSvcFlagID == CIF_ID_DMB) {
			if (tsp[0] == 0x47) {
				cifb->header_on = 0;
				cifb->subch_id = g_nSingleSubchID;
				cifb->ts_size = 188;
				cifb->id_flag = CIF_ID_DMB;
				cifb->ts_ptr -= CIF_HEADER_SIZE;
				return CIFDEC_S__OK;
			}
		}
#endif
...
cifb_alloc_retry:
		...
		cifb = cifb_alloc_buffer(pbTsBuf + CIF_HEADER_SIZE);
		if (!cifb) {
		...
		if (status == CIFDEC_S__OK) {
			...
			case CIF_ID_DAB:
				...
				cifb_enqueue(&cifdec_msc_tspq[q_idx];
#endif
				break;
#ifdef RTV_FIC_CIFMODE_ENABLED
			case CIF_ID_FIC:
	...
	if (cifdec_msc_tspq[0].cnt) { /* Not empty? */
		...
		if (cifb->id_flag == CIF_ID_DMB) /* or DAB+ */
...
{
	UINT i, nTsCnt;
	enum CIF_ID_FLAG_TYPE nIdFlag;
	UINT nDiscardSize = 0;

	nTsCnt = nTsLen / RTV_TSP_XFER_SIZE;

	for (i = 0; i < nTsCnt; i++, pbTsBuf += RTV_TSP_XFER_SIZE) {
		if (pbTsBuf[0] == RTV_CIFDEC_HEADER_SYNC_BYTE) { /* NOT discarded byte */
			nIdFlag = pbTsBuf[1] >> 6;

			if (nFicMscType == 1) {
				if ((nIdFlag == CIF_ID_DMB)
				|| (nIdFlag == CIF_ID_DAB)) {
		...
		for (nIdx = 0; nIdx < RTV_MAX_NUM_USE_SUBCHANNEL; nIdx++) {
			...
			if ((g_nOutDecBufIdxBits & (1<<nIdx)) == 0) {
				...
				case RTV_SERVICE_DMB:
					g_eSingleSvcFlagID = CIF_ID_DMB;
					break;
				case RTV_SERVICE_DAB:
					g_eSingleSvcFlagID = CIF_ID_DAB;
					break;

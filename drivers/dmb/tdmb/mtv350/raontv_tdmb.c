...
{
	U8 lock_stat, rcnt3=0, rcnt2=0, rcnt1=0, rcnt0=0, fb=0,fic_rcnt1=0,fic_rcnt0=0;
	U32 cer_cnt, cer_period_cnt, ret_val, fic_cer_period_cnt, fic_cer_cnt, fic_val;
	...
	RTV_REG_MAP_SEL(FEC_PAGE);
	//For stream Rollback problem
	fb = RTV_REG_GET(0xFB) & 0x01;
	...
	if( lock_stat & 0x01 ) {
		...
		//For stream Rollback problem FIC CER
		fic_rcnt1 = RTV_REG_GET(0x83);
		fic_rcnt0 = RTV_REG_GET(0x84);
		fic_cer_period_cnt = ((fic_rcnt1 << 8) | fic_rcnt0);
		fic_rcnt1 = RTV_REG_GET(0x85);
		fic_rcnt0 = RTV_REG_GET(0x86);
	}
	else {
		cer_period_cnt = 0;
		fic_cer_period_cnt = 0;
    }
  ...
  //For stream Rollback problem
  if(fic_cer_period_cnt != 0) {
    fic_cer_cnt = ((fic_rcnt1 << 8) | fic_rcnt0);
    //RTV_DBGMSG2("[rtvTDMB_GetCER] fic_cer_cnt: %u, fic_cer_period_cnt: %u\n", fic_cer_cnt, fic_cer_period_cnt);

    fic_val = ((fic_cer_cnt * 1000)/fic_cer_period_cnt) * 10;
    if(fic_val > 1200)
      fic_val = 2000;
	...
	else if (!g_fRtvFicOpened && (fic_val < 820) && (fb == 1)) {
      //For stream Rollback problem
      RTV_DBGMSG0("[rtvTDMB_GetCER] Open FIC\n");
      RTV_GUARD_LOCK;

...
int fc8150_scan_status(HANDLE hDevice)
{
	...
	for (i = 0; i < ifagc_timeout; i++) {
		...
		//PRINTF(NULL, "ifagc_timeout : 0x%x\n", data);
		if (data & 0x01)
			break;
	...
	for (; i < ffs_lock_timeout; i++) {
		...
		//PRINTF(NULL, "ffs_lock_timeout : 0x%x\n", data);
	...
	for (i = 0; i < ts_err_free_timeout; i++) {
		...
		//PRINTF(NULL, "ts_err_free_timeout : 0x%x\n", data);
		if (data & 0x01)
			break;

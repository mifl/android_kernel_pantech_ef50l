...
int snd_subsystem_DualPath_poweron(void)
{
	...
	if (is_voice_call_mode) { 	// voice call mode
		...
		snd_subsystem_enHPVRamp(HP_SV_ENABLE);
		snd_subsystem_setHP_NG (0, 0);	// Headphone noise gate disable
#if defined(CONFIG_SKY_EF50L_BOARD) //20130605 hdj  in order to equal dualpath volume and singlepath volume
                snd_sybsystem_SP_cfgInp(SP_MONO_DIFFERENTIAL, /*5*/8);  // Speaker preAMP gain = 4.5dB ->  9dB(8)
#else
                snd_sybsystem_SP_cfgInp(SP_MONO_DIFFERENTIAL, /*5*/7);  // Speaker preAMP gain = 4.5dB
#endif
		snd_subsystem_setSPGain(1);	// Speaker gain = 20dB
#if defined(CONFIG_SKY_EF50L_BOARD) //20130605 hdj  in order to equal dualpath volume and singlepath volume
		snd_subsystem_setSPAtt(/*75*/ /*59*/ 63);	// Speaker volume = -4.5dB -> -15dB (51) -11dB(59) -> -9dB(63)
#else
